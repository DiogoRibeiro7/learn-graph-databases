/**
 * Knowledge graph seeding utilities.
 *
 * This module creates a sample knowledge graph with people, organizations,
 * concepts, documents, and fact-like semantic relationships.
 */
...
export async function seedKnowledgeGraph(driver: Driver): Promise<void> {
  const result = await runQuery(driver, seedCypher);
  const counters = (result as any)?.summary?.counters as any;

  logger.info("Knowledge graph seed summary", {
