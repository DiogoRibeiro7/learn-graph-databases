/* Validate that every instance has an ontology class */
MATCH (e:Entity)
WHERE NOT (e)-[:INSTANCE_OF]->(:OntologyClass)
RETURN e.id AS entityId, labels(e) AS labels;

/* Expand class hierarchy (transitive subclass reasoning) */
...
/* Inference-style: candidate therapies from disease-gene-drug chain */
...
/* Explain inferred therapy paths */
...
/* Organizations are not modeled here; reasoning focus is biomedical class/property semantics */
